$(function(){
	...
	$("#saveBtn").on("click", function(){
		...
		_obj.area				= [];
		_obj.del_file			= [];
		_obj.banner_seq			= $("#banner_seq").val();
		...
		_bannerMainImg.file_seq			 = $(_popBanner).find("input[name=file_seq]").val();
		...
		$.ajax({
			...
			success: function (res) {
				if(res.result == false || res.result == 'false'){
					return alert("수정에 실패하였습니다. 관리자에게 문의 바랍니다.");
				}else{
					alert("수정에 성공하였습니다.");
			...
			error: function (e) {
				alert("수정에 실패하였습니다. 관리자에게 문의 바랍니다.");
	...
	///////	수정페이지
	_prevDisabled(null, 1, false);
	_prevDisabled(null, 2, false); 
	//	미리보기 이벤트 추가
	_preViewEvt(null, 1);
	_preViewEvt(null, 2);
	
	//	이벤트 추가
	var _delBtn = $("#areaAd").children(".area-box").find(".banner-del-btn");
