...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(WebSecurity web) { 
		...
		web.ignoring().antMatchers("/resources/**",  "/favicon.ico", "/error/**", "/admin/", "/upload/**");
	}
	...
	protected void configure(HttpSecurity http) throws Exception{
		http
			.authorizeRequests()						//	authorizeRequests(): 시큐리티 처리에 HttpServletRequest를 이용한다는 것을 의미
				...
				.antMatchers("/admin/login/form").permitAll()
				.anyRequest().hasRole("ADMIN")			//	나머지 요청들은 권한의 종류에 상관 없이 권한이 있어야 접근
			.and() // 로그인 설정
				.formLogin()
				.loginPage("/admin/login/form")				//	로그인 페이지 호출
